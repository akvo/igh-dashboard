...
export function useGeographicDistribution(locationScope = 'Trial Location', statuses) {
  const { actions } = useDashboardStore();
  // Include statuses in the cache key so each filter combination gets its own entry
  const effectiveStatuses = statuses?.length > 0 ? statuses : undefined;
  const cacheKey = getCacheKey('geographicDistribution', { locationScope, statuses: effectiveStatuses });
  const cachedData = actions.getCachedData(cacheKey);

  const { data, loading, error } = useQuery(GET_GEOGRAPHIC_DISTRIBUTION, {
    variables: { scope: locationScope, statuses: effectiveStatuses },
    skip: !!cachedData,
