...
import {
  ...
  writeConflicts,
  hasActiveConflicts,
} from "@/../scripts/content-sync/lib/conflicts.mjs";
...
describe("conflicts", () => {
  it("returns an empty entries list when the file does not exist", () => {
    expect(readConflicts(join(tmp(), "missing.json")).entries).toEqual([]);
  });
  ...
  it("writes empty entries with a null createdAt when given []", () => {
    const p = join(tmp(), "c.json");
    writeConflicts(p, []);
    const out = JSON.parse(readFileSync(p, "utf8"));
