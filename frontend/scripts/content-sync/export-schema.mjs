...
/**
 * Reads the dashboard's content.schema.json and writes a sorted copy
 * to <contentRepoPath>/schema.json. The content repo's schema is the
 * same flat key → constraints map — just sorted by key for
 * diff-stability.
 *
 * Invocation:
 *   node scripts/content-sync/export-schema.mjs <contentRepoPath>
 *
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!target) {
    console.error("Usage: node scripts/content-sync/export-schema.mjs <contentRepoPath>");
    process.exit(1);
  }
