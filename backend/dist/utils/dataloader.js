...
export function createLoaders() {
    ...
    return {
        ...
        diseaseLoader: new DataLoader(async (keys) => {
        ...
        phaseLoader: new DataLoader(async (keys) => {
            const placeholders = keys.map(() => "?").join(", ");
            const rows = db
                .prepare(`
        ...
        developersByCandidateLoader: new DataLoader(async (candidateKeys) => {
        ...
        prioritiesByCandidateLoader: new DataLoader(async (candidateKeys) => {
            const placeholders = candidateKeys.map(() => "?").join(", ");
            const rows = db
                .prepare(`
        ...
        geographiesByCandidateLoader: new DataLoader(async (candidateKeys) => {
            const placeholders = candidateKeys.map(() => "?").join(", ");
            const rows = db
                .prepare(`
        ...
        clinicalTrialsByCandidateLoader: new DataLoader(async (candidateKeys) => {
            const placeholders = candidateKeys.map(() => "?").join(", ");
            const rows = db
                .prepare(`
        ...
        snapshotByCandidateLoader: new DataLoader(async (candidateKeys) => {
            const placeholders = candidateKeys.map(() => "?").join(", ");
            const rows = db
                .prepare(`
