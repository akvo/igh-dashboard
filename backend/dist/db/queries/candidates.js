 ...
 */
export function getCandidates(filter, limit = 20, offset = 0) {
 ...
 */
export function getCandidateByKey(candidate_key) {
    const db = getDatabase();
    const candidate = db
        .prepare(`
    SELECT
 ...
 */
export function getCandidateSnapshot(candidate_key) {
    const db = getDatabase();
    const snapshot = db
        .prepare(`
