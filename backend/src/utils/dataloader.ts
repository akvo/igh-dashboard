...
export function createLoaders() {
  ...
  return {
    ...
    diseaseLoader: new DataLoader<number, DimDisease | null>(
      async (keys) => {
    ...
    developersByCandidateLoader: new DataLoader<number, DimDeveloper[]>(
      async (candidateKeys) => {
    ...
    prioritiesByCandidateLoader: new DataLoader<number, DimPriority[]>(
      async (candidateKeys) => {
        const placeholders = candidateKeys.map(() => "?").join(", ");
        const rows = db
          .prepare(
    ...
    clinicalTrialsByCandidateLoader: new DataLoader<
      number,
      FactClinicalTrialEvent[]
    >(async (candidateKeys) => {
      const placeholders = candidateKeys.map(() => "?").join(", ");
    ...
    snapshotByCandidateLoader: new DataLoader<
      number,
    ...
    >(async (candidateKeys) => {
      ...
      const map = new Map(
        rows.map((r) => [r.candidate_key as number, r])
      );
